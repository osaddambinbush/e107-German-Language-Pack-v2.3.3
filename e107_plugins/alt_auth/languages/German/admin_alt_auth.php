<?php

// Bing-Translated Language file 
// Generated for e107 v2.x by the Multi-Language Plugin
// https://github.com/e107inc/multilan

define("LAN_ALT_1", "Primäre Zulassung Typ");
define("LAN_ALT_2", "Update-Einstellungen");
define("LAN_ALT_3", "Wählen Sie alternative Zulassung");
define("LAN_ALT_4", "Konfigurieren von Parametern für");
define("LAN_ALT_5", "Zulassung-Parameter konfigurieren");
define("LAN_ALT_6", "Fehlgeschlagene Verbindung Aktion");
define("LAN_ALT_7", "Wenn ein Verbindungsfehler auf dem primären Zulassung-Typ (und nicht der lokalen e107 DB), wie soll man das umgehen?");
define("LAN_ALT_8", "Sekundäre Zulassung Typ");
define("LAN_ALT_9", "Dies wird verwendet, wenn die primäre Zulassung-Methode den Benutzer nicht finden können");
define("LAN_ALT_10", "Benutzer-Login-Name-Feld");
define("LAN_ALT_11", "Benutzer-Passwort-Feld");
define("LAN_ALT_12", "Benutzer e-Mail-Feld");
define("LAN_ALT_13", "Verstecken Sie Email? Feld");
define("LAN_ALT_14", "Display Name Benutzerfeld");
define("LAN_ALT_15", "Richtigen Namen Benutzerfeld");
define("LAN_ALT_16", "Benutzerfeld Custom-Titel");
define("LAN_ALT_17", "Unterschriftsfeld");
define("LAN_ALT_18", "Avatar-Feld");
define("LAN_ALT_19", "Foto-Feld");
define("LAN_ALT_20", "Feld 'Datum' beitreten");
define("LAN_ALT_21", "Ban-Status-Feld");
define("LAN_ALT_22", "Klassenfeld Mitgliedschaft");
define("LAN_ALT_24", "Kennwortfeld Salz");
define("LAN_ALT_25", "(manchmal kombiniert mit Passwort für zusätzliche Sicherheit)");
define("LAN_ALT_26", "Datenbanktyp:");
define("LAN_ALT_27", "Um einen Feldwert in die lokale Datenbank zu übertragen, geben Sie den Feldnamen in das entsprechende Feld unten. (Benutzername und Passwort sind immer übernommen)<br>Lassen Sie das Feld leer, damit es nicht überhaupt übertragen werden");
define("LAN_ALT_29", "Auth-Methoden");
define("LAN_ALT_30", "Konfigurieren");
define("LAN_ALT_31", "Hauptkonfigurationsdatei");
define("LAN_ALT_32", "Server:");
define("LAN_ALT_33", "Benutzername:");
define("LAN_ALT_34", "Passwort:");
define("LAN_ALT_35", "Datenbank:");
define("LAN_ALT_36", "Tabelle:");
define("LAN_ALT_37", "Username-Feld:");
define("LAN_ALT_38", "Passwort-Feld:");
define("LAN_ALT_39", "Tabellen-Prefix:");
define("LAN_ALT_40", "Testen Sie den Datenbankzugriff");
define("LAN_ALT_41", "(mittels oben Anmeldeinformationen)");
define("LAN_ALT_42", "Benutzername und Passwort eingegeben, wird dieser Benutzer auch validiert werden");
define("LAN_ALT_43", "Verbindung zur Datenbank erfolgreich");
define("LAN_ALT_44", "Verbindung zur Datenbank fehlgeschlagen");
define("LAN_ALT_45", "Username-Lookup erfolgreich");
define("LAN_ALT_46", "Uername Lookup fehlgeschlagen");
define("LAN_ALT_47", "Test");
define("LAN_ALT_48", "Vorherige Validierung");
define("LAN_ALT_49", "Username =");
define("LAN_ALT_50", "Passwort =");
define("LAN_ALT_51", "(leer)");
define("LAN_ALT_52", "Authentifizierung fehlgeschlagen-");
define("LAN_ALT_53", "unbekannter Ursache");
define("LAN_ALT_54", "kann nicht verbinden mit DB / Dienstleister");
define("LAN_ALT_55", "Ungültiger Benutzer");
define("LAN_ALT_56", "falsches Kennwort");
define("LAN_ALT_57", "Methode nicht verfügbar");
define("LAN_ALT_58", "Erfolgreiche Authentifizierung");
define("LAN_ALT_59", "Abgerufene Parameter:");
define("LAN_ALT_60", "Erweiterte Benutzerfelder");
define("LAN_ALT_61", "Ermöglichen");
define("LAN_ALT_62", "Feldname");
define("LAN_ALT_63", "Beschreibung");
define("LAN_ALT_64", "Typ");
define("LAN_ALT_65", "Alternative-Authentifizierung");
define("LAN_ALT_66", "Dieses Plugin ermöglicht Alternative Authentifizierungsmethoden.");
define("LAN_ALT_67", "Konfigurieren von Alt-auth");
define("LAN_ALT_68", "Alt-Auth-Dienst ist jetzt eingerichtet.  Jetzt müssen Sie Ihre bevorzugte Methode zu konfigurieren.");
define("LAN_ALT_69", "");
define("LAN_ALT_70", "Keine");
define("LAN_ALT_71", "WAHR/FALSCH");
define("LAN_ALT_72", "Grossbuchstaben");
define("LAN_ALT_73", "Kleinbuchstaben");
define("LAN_ALT_74", "Obere erste");
define("LAN_ALT_75", "Oberen Wörter");
define("LAN_ALT_76", "Einschränkung der Benutzer-Klasse (ein numerischer Wert - NULL oder leer für alle)");
define("LAN_ALT_77", "Nur Benutzer in dieser Klasse (in der Datenbank oben) ist der Zugriff gestattet");
define("LAN_ALT_78", "Fehlgeschlagener Aktion");
define("LAN_ALT_79", "Wenn Benutzer in primären DB existiert, jedoch ein falsches Kennwort eingibt, wie sollte, die werden behandelt?");
define("IMPORTDB_LAN_2", "Nur-Text");
define("IMPORTDB_LAN_3", "Joomla, gesalzen");
define("IMPORTDB_LAN_4", "Mambo gesalzen");
define("IMPORTDB_LAN_5", "SMF (SHA1)");
define("IMPORTDB_LAN_6", "Generische SHA1");
define("IMPORTDB_LAN_7", "MD5 (E107 Original)");
define("IMPORTDB_LAN_8", "E107 gesalzen (option 2.0)");
define("IMPORTDB_LAN_12", "PHPBB2/PHPBB3 gesalzen");
define("IMPORTDB_LAN_13", "WordPress, gesalzen");
define("IMPORTDB_LAN_14", "Magento, gesalzen");
define("LAN_ALT_FALLBACK", "Verwendung sekundärer Zulassung");
define("LAN_ALT_FAIL", "Fehlgeschlagener login");
define("LAN_ALT_UPDATESET", "Update-Einstellungen");
define("LAN_ALT_UPDATED", "Einstellungen aktualisiert");
define("LAN_ALT_AUTH_HELP", "Dies sind die Einstellungen für alle Authentifizierungsmethoden und bestimmen die zu ergreifenden Maßnahmen<br><br>Benutzerfeld erweiterte Auswahl bestimmt, welche <i>möglicherweise</i> hinzugefügt/aktualisiert, wenn sich ein Benutzer anmeldet - weitere Konfiguration erforderlich für die spezifische Authentifizierungsmethode ist.");
define("LAN_ALT_VALIDATE_HELP", "Überprüfen Sie die Einstellungen mithilfe der \'Test Database Access\' Abschnitt um zu versuchen und ein Benutzers zu überprüfen - das genau den gleichen Prozess verwendet, als wenn ein Benutzer versucht, anzumelden, und bestätigt, ob Ihre Einstellungen korrekt sind.<br>Wenn Sie einige Parameter in der Tabelle User bei erfolgreicher Anmeldung kopiert werden konfiguriert haben, werden diese ebenfalls aufgelistet.");
define("LAN_ALT_COPY_HELP", "Sie können Felder in der Benutzerdatenbank aus der entfernten Datenbank kopieren, indem Sie die entsprechenden Namen eingeben auswählen.<br><br>");
define("LAN_ALT_CONVERSION_HELP", "Für einige Felder wählt die Dropdown-Box rechts vom Eintrag im Feld eine Konvertierung, die auf den Wert aus der entfernten Datenbank lesen angewandt werden kann; Wenn \'none\' ist aktiviert, der Wert kopiert wie empfangen. Konvertierungen sind:[b]Wahr/falsch[b] - die Worte \'TRUE\' und \'FALSE\' (und ihre untere/gemischt Fall Entsprechungen) werden konvertiert, die boolesche Werte 1 und 0 (null).<br>[b]Großbuchstaben[b] - alle Buchstaben in Großbuchstaben umgewandelt<br>[b]Kleinbuchstaben[b] - alle Buchstaben in Kleinbuchstaben konvertiert werden<br>[b]Obere erste[b] -das erste Zeichen in Großbuchstaben konvertiert wird<br>[b]Obere Worte[b] - der erste Buchstabe jedes Wortes ist in Großbuchstaben konvertiert.<br><br><br>");





